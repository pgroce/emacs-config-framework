...
This configuration replaces the =\*scratch\*= buffer with a file, =scratch.el=.[fn:: Conceptually; the =\*scratch\*= is retained if you want to use it.] =scratch.el= is automatically loaded and, by default, is the first buffer visited, just like =\*scratch\*=. However, it is a first-class configuration file; on startup, =scratch.el= is loaded after all other configuration files. =scratch.el= is, thus, a persistent =\*scratch\*=.
